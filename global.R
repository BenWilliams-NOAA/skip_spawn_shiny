...
#' @param vonb_sd provides error about estimates (not currently used)
...
population <- function(age = 50, meanpop = 6.5, sigr = 1, failure = NULL,
                       type = NULL, m = 0.098, fish = 0.117, s50 = 3.7, bin = 30){
...
plot_pd <- function(sb, age, bin){
  ...
  if(age>bin | "gam_bin" %in% sb$Model){
    sb %>%
      ...
      facet_wrap(~group) +
      scale_fill_manual(values = c("#7E1900", "#B48A2C", "#E5E598", "#8CDED9")) +
      ylab("% difference from true") +
      geom_hline(yintercept = 0, lty = 3)
  } else {
    sb %>%
      filter(!(id %in% c("b0", "plus"))) %>%
      mutate(group = case_when(id == "n" ~ "full",
                               T ~ "plus"),
             Model = gsub("\\_.*", "", Model)) %>%
      group_by(year) %>%
...
plot_mature <- function(data, type = NULL){

  # if(is.null(type)){
  if("glm_bin" %in% data$Model){
    data %>%
      ggplot(aes(age, fit)) +
    ...
    data %>%
      ...
      theme(legend.position = c(0.8, 0.2)) +
      scale_color_manual("Model", values = c("#7E1900",  "#D1ECC9", "#1A3399")) +
      scale_fill_manual("Model", values = c("#7E1900", "#D1ECC9", "#1A3399")) +
      xlab("Age") +
      ylab("Maturity") +
      expand_limits(x = 0, y = 0)
  }
  ...
  #     ungroup() %>%
  #     ggplot(aes(age, length, size = Difference, fill = cols)) +
  #     geom_point(pch = 21, alpha = 0.7) +
  #     scale_size_area(guide = FALSE) +
  #     facet_wrap(~Model) +
  #     scale_fill_manual("Residuals", values = c("#8CDED9", "#7E1900"),
  ...
  # data %>%
  #       ggplot(aes(age, length)) +
  #       geom_point(aes(color = fit), size = 4) +
  #       facet_wrap(~Model) +
  #       theme(legend.position = c(0.85, 0.2)) +
