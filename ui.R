             ...
             navbarMenu("Instructions",
                        tabPanel("Setup",
                                 mainPanel("This Shiny app is designed to explore different estimation models for fish that exhibit skip spawning." ,
                                           ...
                                           br(),
                                           br(),
                                           "The of Inputs and Models tabs have parameters that can be adjusted in order to explore different maturation rates, variable skip spawning etc.",
                                           br(),
                                           "These simulations are based upon having complete sample sizes for all ages, an unlikely scenario.
                       Therefore a 'weighted' age-based maturity model is available.
                       This scenario allows for the user to define sample sizes for each age as well as data weights if desired.",
                                           br(),
                                           "If no skip spawning is present then a GLM and GAM will produce very similar results."
                                 )
                        )
             ),
             navbarMenu("Inputs",
                                 ...
                                 sidebarLayout(
                                   sidebarPanel(
                                     ...
                                     sliderInput("nsim",
                                                 ...
                                                 max = 1000,
                                                 value = 500)),
                                   mainPanel(dt_output("Samples", "sample_tbl"))
                                 )
                        ),
                        tabPanel("Skip spawning", fluid = TRUE,
                                 sidebarLayout(
                                   sidebarPanel(
                                     ...
                                     sliderInput("maxskip", "Max skip age:", min = 0, max = 100, value = 25),
                                     br(),
                        ...
                        tabPanel("Length", fluid = TRUE,
                                 sidebarLayout(
                                   sidebarPanel(
                                     ...
                                     sliderInput("b1_l", "intercept:", min = 0, max = 100, value = 65, step = 1),
